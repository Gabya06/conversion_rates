"""
script to calculate and populate conversion rates into Conversion Rate object in SalesForce (not Windows machine, locally)
"""
...
# import pyodbc # for salesforce backups query windows machine login
import beatbox # to upload rates to SF
...
for i in t_dates:
    ...
    for j in stages[1:-1]:
        ...
        temp_rates.ix[j]['ConversionRate', 'Numerator', 'Denominator'] = [str(r) for r in res]
    ...
    # to be able to update to SF
    temp_rates.columns = temp_rates.columns + '__c'

    temp_rates['type'] = 'Conversion_Rate__c'
    conv_obj = svc.describeSObjects('Conversion_Rate__c')
    conv_fields = conv_obj[0].fields
    conv_rates_obj = temp_rates.to_dict('records')


    # Conversion_Rate__c
    print "\n conversion rates object for SF: \n", conv_rates_obj

    # create new rows in conversion rate object
    results = svc.create(conv_rates_obj)
    for rec in results:
        if rec['success'] == True:
            print "Success creating conversion rate row"
        else:
            err_stat = rec['errors'][0]['statusCode']
            print err_stat

    print "***** SUCCESSFULLY ADDED ROWS TO CONVERSION RATE OBJECT IN SALESFORCE *****"
